...
const FundDepositPage: React.FC = () => {
    ...
    const handleApprove = async () => {
        if (!signer || !amount || !denominationAsset || !comptrollerProxyAddress) return;
        ...
        try {
            ...
            const comptrollerContract = new ethers.Contract(comptrollerProxyAddress, COMPTROLLER_ABI, signer);
            const vaultProxyAddress = await comptrollerContract.getVaultProxy();

            const tx = await investmentAssetContract.approve(vaultProxyAddress, investmentAmount);
            setTxHash(tx.hash);
            await tx.wait();
    ...
    const handleDeposit = async () => {
        if (!signer || !amount || !comptrollerProxyAddress) return;
    ...
    return (
        <div className="container mx-auto p-4 md:p-8">
            <div className="mb-8 mt-4">
                ...
                <p className="text-lg text-gray-500 truncate">基金地址: {comptrollerProxyAddress}</p>
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="bg-white p-8 rounded-2xl shadow-lg h-fit">
                    ...
                    {denominationAsset ? (
                        ...
                        <div className="text-center text-gray-500">正在加載基金資訊...</div>
                    )}
                </div>
            </div>
        </div>
